"""Xception backbone, vendored from DeepfakeBench (SCLBD/DeepfakeBench,
training/networks/xception.py — CC BY-NC 4.0), itself adapted from
https://github.com/ondyari/FaceForensics. Architecture code only, no
training/registry scaffolding from the original framework — this project
loads a single fixed pretrained checkpoint (F3-Net) directly.

Kept structurally identical to the original so the F3-Net checkpoint's
state_dict loads with matching parameter names and shapes.
"""
...
class Xception(nn.Module):
    """Xception optimized for ImageNet, per https://arxiv.org/pdf/1610.02357.pdf"""
